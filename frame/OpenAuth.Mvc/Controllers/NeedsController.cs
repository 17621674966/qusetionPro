...
namespace OpenAuth.Mvc.Controllers
{
    public class NeedsController : Controller
    {
        ...
        public T_Needs_InfoApp appInformation { get; set; }

        public T_Needs_SharesApp appShare { get; set; }



        public T_Needs_AttachApp appAttach { get; set; }
        ...
        {
            string informationTypeGuid = ConfigurationManager.AppSettings["needsTypeGuid"].ToString();
        ...
        public string Add(string obj)
        {
            ...
            try
            {
                T_Needs_Info model = JsonHelper.Instance.Deserialize<T_Needs_Info>(obj);
                ...
                T_Needs_Shares share = new T_Needs_Shares();
                share.RowGuid = Guid.NewGuid().ToString();
                share.NeedsGuid = model.RowGuid;
        ...
        public string AddInformationImg(string guid)
        {
            var result = new Response();
            string userGuid = ConfigurationManager.AppSettings["addUserGuid"].ToString();
            string fileAbsolutePath = ConfigurationManager.AppSettings["fileUploadObsolutePathNeeds"].ToString();
            string filePath = ConfigurationManager.AppSettings["fileUploadPathNeeds"].ToString();
            try
            {
                ...
                else
                {
                    for (int i = 0; i < HttpContext.Request.Files.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            T_Needs_Attach attach = new T_Needs_Attach();
        ...
        public string GetList(string title, string addUserName, string beginTime, string endTime, string status,int type)
        {
            ...
            try
            {
                var res = from a in appInformation.UnitWork.Find<T_Needs_Info>(x => x.ID > 0)
                ...
                //拼接表达式
                Expression<Func<NeedsModel, bool>> exp = PredicateBuilder.True<NeedsModel>();
                ...
                if (type!=-1)
                {
                    exp = exp.And(x=>x.Type==type);
                }


                IQueryable<NeedsModel> iquery = ExpandableHelper.GetExpandable(res.Where(exp));
                List<NeedsModel> list = iquery.OrderByDescending(x => x.AddTime).Skip(pageSize * (pageNo - 1)).Take(pageSize).ToList();
                list.ForEach(x =>
                {
        ...
        public string DeleteInformation(string rowGuid)
        {
            ...
            try
            {
                T_Needs_Info info = appInformation.Repository.FindSingle(x => x.RowGuid.Equals(rowGuid));
                if (info == null)
                {
                    result.Code = 500;
        ...
        public ActionResult Detail(string rowGuid)
        {
            ...
            List<T_Needs_Attach> listAttach = appAttach.Repository.Find(x => x.NeedsGuid.Equals(rowGuid)).ToList();
