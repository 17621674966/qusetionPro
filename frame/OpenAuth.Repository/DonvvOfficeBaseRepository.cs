...
{
   public  class DonvvOfficeBaseRepository<T> :IRepository<T> where T : BaseEntity
    {
       protected DonvvOfficeDbContext Context = new DonvvOfficeDbContext();
       ...
       /// <summary>
        ...
        {
            if (string.IsNullOrEmpty(entity.RowGuid))
            {
        ...
        private IQueryable<T> Filter(Expression<Func<T, bool>> exp)
        {
            ...
            if (exp != null)
                dbSet = dbSet.AsExpandable().Where(exp);
            return dbSet;
        }
